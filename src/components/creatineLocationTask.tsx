...
const isTimePassedForDay = (reminderTime: string): boolean => {
  const now = new Date()
  const [targetHour, targetMinute] = reminderTime.split(":").map(Number)
  const currentTotalMinutes = now.getHours() * 60 + now.getMinutes()
  const targetTotalMinutes = (targetHour ?? 0) * 60 + (targetMinute ?? 0)
...
// ─── Shared reminder evaluation logic ────────────────────────────────────────

interface ReminderEvalInput {
  latitude: number
  longitude: number
...
/**
 * Evaluates whether a creatine reminder should fire given the current
 * location, settings, and already-shown state. Returns true if a
 * notification was sent.
 */
const evaluateAndFireReminder = async ({
  latitude,
  longitude,
  settings,
  userId,
}: ReminderEvalInput): Promise<boolean> => {
  if (!settings.locationBasedReminder || !settings.reminderLocation) return false

  const today = new Date().toDateString()
  const lastTaken = await AsyncStorage.getItem(`creatineLastTaken_user_${userId}`)
  ...
  if (checkTime && checkLocation) {
    if (isTimePassedForDay(settings.reminderTime) && locationConditionMet) {
      shouldFire = true
...
TaskManager.defineTask(
  ...
  async ({
    ...
    try {
      ...
      await evaluateAndFireReminder({ latitude, longitude, settings, userId })
    } catch (err) {
      await writeDebugLog(
...
export const triggerImmediateLocationCheck = async (): Promise<boolean> => {
  try {
    ...
    return await evaluateAndFireReminder({ latitude, longitude, settings, userId })
  } catch {
    return false
  }
}
