...
export const photoApi = {
   ...
   */
  getPhotosInRange: async (startDate, endDate) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || "Failed to get photos in range")
      }
   ...
   */
  getPhotosGroupedByDate: async (days = 90) => {
    try {
      const API_BASE_URL = getServerUrl()
      const response = await authenticatedFetch(
      ...
      if (!response.ok) {
        throw new Error(data.error || "Failed to get grouped photos")
      }
  ...
  comparePhotos: async (date1, date2) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || "Failed to compare photos")
      }
