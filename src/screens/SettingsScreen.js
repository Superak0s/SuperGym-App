...
export default function SettingsScreen() {
  ...
  const [showTimeBetweenSetsModal, setShowTimeBetweenSetsModal] =
    useState(false)
  const [tempTimeBetweenSets, setTempTimeBetweenSets] = useState("")
  ...
  const [selectedDayToReset, setSelectedDayToReset] = useState(null)
  ...
  const [creatineLocationBasedEnabled, setCreatineLocationBasedEnabled] =
    useState(false)
  const [creatineReminderTime, setCreatineReminderTime] = useState(new Date())
  const [showCreatineTimePicker, setShowCreatineTimePicker] = useState(false)
  const [creatineDefaultGrams, setCreatineDefaultGrams] = useState("5")
  ...
  const [showCreatineSettings, setShowCreatineSettings] = useState(false)
  ...
  const loadCreatineSettings = async () => {
    try {
      ...
      try {
        const status = await bodyTrackingApi.getCreatineStatus()
  ...
  const handleResetSingleDay = (dayNumber) => {
    const day = workoutData.days.find((d) => d.dayNumber === dayNumber)
    const dayTitle = day ? day.title : `Day ${dayNumber}`
    const hasActiveSession = !!workoutStartTime
  ...
  const getLockedDaysCount = () =>
    Object.keys(lockedDays).filter((day) => lockedDays[day]).length
                  ...
                  <View style={styles.card}>
                    ...
                    >
                      <View style={{ flex: 1 }}>
                        ...
                        <Text style={styles.settingDescription}>
                          {BATTERY_PRESETS[batteryPreset]?.label ||
                            "Medium Impact"}
                          {" - "}
                          {BATTERY_PRESETS[batteryPreset]?.description ||
                            "Checks every 10 min"}
                        </Text>
                      </View>
            ...
            <View style={styles.section}>
              ...
              <View style={styles.card}>
                <View style={styles.infoRow}>
                  ...
                  <Text style={styles.infoValue}>
                    {formatTime(serverAnalytics.averageTimeBetweenSets)}
                  </Text>
                </View>
                <View style={styles.divider} />
                <View style={styles.infoRow}>
                ...
                <View style={styles.infoRow}>
                  ...
                  <Text style={styles.infoValue}>
                    {serverAnalytics.totalSetsCompleted}
                  </Text>
                </View>
              </View>
              <Text style={styles.helperText}>
            ...
            getDaysWithActivity().map((day) => (
              ...
              >
                <View style={styles.dayListItemContent}>
                  ...
                  <Text style={styles.dayListItemSubtitle}>{day.title}</Text>
                </View>
